...
window.checkForUpdate = function () {
  ...
  navigator.serviceWorker.getRegistration().then((reg) => {
    if (reg) {
      reg.update().then(() => showToast("Checking for updates…"));
    }
  });
};
...
window.applyUpdate = function () {
  ...
  if (pendingSW) {
    ...
    // No pending SW — force check and reload
    navigator.serviceWorker.getRegistration().then((reg) => {
      if (reg?.waiting) {
    ...
    <div style="display:flex;flex-direction:column;gap:12px">
      ...
      <div style="display:flex;gap:8px;flex-wrap:wrap">
        <button class="btn btn-secondary btn-sm" style="flex:1"
          onclick="openHarvestCompare(\'${id}\')">📷 Compare to Trail Cam</button>
        ${isOwner ? `
          <button class="btn btn-secondary btn-sm" onclick="openEditHarvest(\'${id}\')">✏️</button>
          <button class="btn btn-danger btn-sm" onclick="deleteHarvest(\'${id}\')">🗑</button>
        ` : ""}
      </div>
    </div>
  `;
...
function renderTcFeed(snap) {
  ...
  docs.forEach(d => {
    ...
    const key = data.batchId || (data.uid + "_" + created.toDateString());
    if (!grouped[key]) {
      grouped[key] = { ts:created.getTime(), label:data.batchLabel||(data.uploaderName+" · "+created.toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"})), uploader:data.uploaderName||"Unknown", initials:data.uploaderInitials||"?", color:data.uploaderColor||"#556B2F", photos:[] };
      order.push(key);
    }
...
function feedPostCard(post) {
  ...
  return `
    <div style="background:var(--forest-card);border:1px solid var(--card-border);
      ...
      <div style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:8px" id="feed-reactions-${post.id}">
        ${renderReactionBadges(reactions, post.id, "feed")}
      </div>
      ${userProfile && !userProfile.isGuest ? `
        <div style="display:flex;gap:5px;margin-bottom:10px;flex-wrap:wrap">
          ${REACTIONS_LIST.map(e => `
            <button onclick="addFeedReaction('${post.id}','${e}')"
              style="background:rgba(255,255,255,0.06);border:1px solid var(--card-border);
                     border-radius:20px;padding:4px 9px;font-size:14px;cursor:pointer;
                     transition:all 0.2s;font-family:var(--font-sans)"
              onmouseover="this.style.borderColor='var(--gold-dim)'"
      ...
      <button onclick="toggleFeedComments('${post.id}')"
        style="background:none;border:none;color:var(--text-muted);font-size:12px;
               cursor:pointer;font-family:var(--font-sans);padding:0;margin-bottom:8px">
        💬 ${commentCount} comment${commentCount !== 1 ? "s" : ""} — tap to ${commentCount ? "view" : "add"}
      </button>
...
async function renderMyKillsScreen() {
  const content = document.getElementById("mykills-content");
  if (!content) return;

  if (!userProfile || userProfile.isGuest) {
    content.innerHTML = `<div style="padding:32px;text-align:center;color:var(--text-muted)">Sign in to view your kills.</div>`;
    return;
  }
  ...
  try {
    ...
    const pts       = userData.killPoints  || 0;
    const tier      = getTierForPoints(pts);
    const nextTier  = getNextTier(pts);
    const progress  = nextTier ? Math.round((pts / nextTier.pts) * 100) : 100;

    // Load harvests
    const q    = query(collection(db, "harvests"), where("uid","==",userProfile.uid), orderBy("harvestDate","desc"));
    const snap = await getDocs(q);

    // Species breakdown
    const breakdown = {};
    snap.docs.forEach(d => {
      const sp = d.data().species || "other";
      ...
      <div style="padding:16px;display:flex;flex-direction:column;gap:14px">
        ...
        <div class="card card-highlight" style="text-align:center;padding:20px">
          ...
          <div style="font-size:13px;color:var(--text-muted);margin-bottom:14px">
            ${pts} kill points · ${kills} total harvest${kills !== 1 ? "s" : ""}
          </div>
          ...
          ${nextTier ? `
            ...
            <div style="color:var(--gold);font-size:13px">Maximum rank achieved. You are a legend.</div>`}
        </div>

        <!-- How to rank up — collapsible -->
        <div>
          ...
          <div id="rankup-content" class="hidden"
            style="background:rgba(14,10,4,0.92);border:1px solid var(--gold-dim);
                   border-top:none;border-bottom-left-radius:var(--radius-lg);
                   border-bottom-right-radius:var(--radius-lg);padding:14px;margin-bottom:10px">
            <div style="margin-bottom:14px">
            ...
            <div>
              ...
              ${[
                ["🦌 Buck Deer", 10], ["🦌 Doe Deer", 5],
                ["🦃 Turkey Tom", 10], ["🦃 Turkey Jake/Hen", 5],
                ["🐻 Bear", 15], ["🦆 Waterfowl", "2 per bird"],
                ["🐿️ Small Game", "1 per animal"], ["🐺 Predator (Coyote/Wolf/Fox)", 3]
              ].map(([label, pts]) => `
                <div style="display:flex;justify-content:space-between;padding:5px 0;
                            border-bottom:1px solid rgba(196,169,106,0.07)">
                  <span style="font-size:13px;color:var(--text-muted)">${label}</span>
                  <span style="font-size:13px;color:var(--gold);font-weight:600">${pts}</span>
                </div>`).join("")}
            </div>
          </div>
        </div>
        ...
        <div>
          <button class="tc-row-header" id="breakdown-toggle"
            ...
            <div style="flex:1;text-align:left">
              ...
              <div style="font-size:12px;color:var(--text-muted)">${kills} total harvests</div>
            </div>
        ...
        <div>
          <div style="font-size:12px;color:var(--text-muted);letter-spacing:0.5px;
                      text-transform:uppercase;margin-bottom:10px">Your Harvest History</div>
          ${snap.docs.length === 0
            ...
            : snap.docs.map(d => {
                ...
                const score   = h.rackScore ? ` · ${h.rackScore}" B&C` : "";
                return `
                  <div style="margin-bottom:${isExp ? "0" : "8px"}">
                    <button class="tc-row-header ${isExp ? "expanded" : ""}"
                      onclick="toggleMyKillRow('${h.id}')" style="margin-bottom:0">
                      <span style="font-size:24px">${sp.icon}</span>
                      <div style="flex:1;min-width:0">
                        <div style="font-size:13px;font-weight:600;color:var(--text-warm)">${sp.label}</div>
                        <div style="font-size:11px;color:var(--text-muted)">${dateStr}${weight}${score}</div>
                      </div>
                    ...
                    ${isExp ? `
                      <div style="background:rgba(14,10,4,0.92);border:1px solid var(--gold-dim);
                        ...
                        ${h.notes ? `<div style="font-size:13px;color:var(--text-muted);margin-bottom:10px">${h.notes}</div>` : ""}
                        <button class="btn btn-primary btn-sm btn-full" onclick="goHarvest()">
                          View in Harvest Log
                        </button>
                      </div>` : ""}
                  </div>`;
